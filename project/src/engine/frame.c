...
/*   Updated: 2024/08/31 20:15:23 by gabriel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	frame->image = mlx_new_image(engine->mlx, engine->screen.x, engine->screen.y);
	if (frame->image == NULL)
...
for(int x = 0; x < 640; ++x)
{
	...
	*/
	...
	if (frame->endian == 1)
	{
		...
		frame->buffer[pixel_pos + 1] = color.r;
		frame->buffer[pixel_pos + 2] = color.g;
		frame->buffer[pixel_pos + 3] = color.b;
	}
	else
	{
		frame->buffer[pixel_pos + 0] = color.b;
		frame->buffer[pixel_pos + 1] = color.g;
		frame->buffer[pixel_pos + 2] = color.r;
